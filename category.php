...
$cat_pid = isset($_REQUEST['pid'])?intval($_REQUEST['pid']):'';
...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update','selled'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if ($cat_pid){
        $smarty->assign('categories',       get_categories_tree($cat_pid)); // 分类树
    }else{
        $smarty->assign('categories',       get_categories_tree($cat_id)); // 分类树
    }
    $smarty->assign('cat_id',       $cat_id); // 分类ID
    //echo "<pre>";print_r(get_categories_tree($cat_id));exit();
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.sell_number, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ";
     $sql .= "WHERE $where $ext ORDER BY $sort $order";
