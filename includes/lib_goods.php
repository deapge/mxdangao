...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    //echo "<pre>";print_r($type_goods[$type]);exit();
...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
            ...
            'ORDER BY a.sort_order, g.goods_attr_id, g.attr_price';
    $res = $GLOBALS['db']->getAll($sql);
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    if ($area_ads[$cat_id])
        $cat['area_ads']   = getads($area_ads[$cat_id], 3);
    return $cat;
}
...
{
    $time = gmtime();
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('ad') . " where position_id=".$cat." and start_time <= '" . $time . "' AND end_time >= '" . $time . "' ORDER BY ad_id ASC limit ".$num;
    $res = $GLOBALS['db']->getAll($sql);
    $arr = array();
    foreach ($res AS $idx => $row)
    {
